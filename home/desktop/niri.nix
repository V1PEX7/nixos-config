...
let
  ...
  hyprToHex = c: "#" + builtins.substring 5 8 c;

  activeBorder = hyprToHex t.hypr.active_border;
  inactiveBorder = hyprToHex t.hypr.inactive_border;
  shadowColor = hyprToHex t.hypr.shadow;
  backdropColor = t.bg;
in
{
  home.file.".config/niri/config.kdl".text = ''
    ...
    spawn-at-startup "wl-clip-persist" "--clipboard" "regular" "--reconnect-tries" "0"
    ...
    window-rule {
        match app-id=r#"^firefox$"# title=r#".*Private Browsing.*"#
    ...
    window-rule {
        match title=r#"^(Share Screen)$"#
        open-floating true
    }

    window-rule {
        match title=r#"^(Screen Share)$"#
        open-floating true
    }
    ...
    window-rule {
        match app-id=r#"^chromium-browser$"# title=r#"^(Picture-in-Picture)$"#
        open-floating true
    }
    ...
    binds {
        ...
        Mod+W hotkey-overlay-title="Browser: chromium" { spawn "chromium"; }
        Mod+S hotkey-overlay-title="Browser: firefox" { spawn "firefox"; }

        Mod+Space hotkey-overlay-title="Launcher" { spawn "fuzzel"; }
        Mod+V hotkey-overlay-title="Clipboard" { spawn-sh "cliphist list | fuzzel --dmenu | cliphist decode | wl-copy"; }
        ...
        Mod+Shift+D { maximize-window-to-edges; }
